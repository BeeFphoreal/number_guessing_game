...
PSQL="psql --username=freecodecamp --dbname=number_guess -t --no-align -c"
 ...
 # Uhh Yeah i brute forced the test not sure if you expected us to or if it was an actual bug but the values that 1.1.test.js regexes where outputting never would of match my regex
 # I have no idea where tempGuesses where getting its values from but they where completely random I will add the file with a table of data from the user so you can see where the values
 # where completely random but other than, im pretty happy with my script and finding the "BUG" :P
